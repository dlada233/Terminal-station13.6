...
/obj/item/autopsy_scanner/interact_with_atom(atom/interacting_with, mob/living/user)
	...
	if ((HAS_TRAIT(user, TRAIT_CLUMSY) || HAS_TRAIT(user, TRAIT_DUMB)) && prob(50))
		user.visible_message(span_warning("[user]分析地板的生命特征!"), \
							span_notice("你愚蠢地试图分析地板上的生命体征!"))
		to_chat(user, "[span_info("地板的扫描结果:\n\t整体状况: <b>伤情</b>")]\
				\n[span_info("Key: <font color='#00cccc'>窒息伤</font>/<font color='#00cc66'>毒素伤</font>/<font color='#ffcc33'>烧伤</font>/<font color='#ff3333'>暴力伤</font>")]\
				\n[span_info("\t伤害细节: <font color='#66cccc'>0</font>-<font color='#00cc66'>0</font>-<font color='#ff9933'>0</font>-<font color='#ff3333'>0</font>")]\
				\n[span_info("体温: ???")]")
		return

	user.visible_message(span_notice("[user]扫描[M]的尸体."))
	to_chat(user, span_deadsay("[icon2html(src, user)]分析中."))
...
/obj/item/autopsy_scanner/proc/scan_cadaver(mob/living/carbon/human/user, mob/living/carbon/scanned)
	...
	for(var/obj/item/bodypart/bodyparts as anything in scanned.bodyparts)
		...
		autopsy_information += "暴力伤: [bodyparts.brute_dam] | 烧伤: [bodyparts.burn_dam]<br>"
		if(!bodyparts.wounds)
			continue
	...
	if(HAS_TRAIT(scanned, TRAIT_HUSK))
		autopsy_information += "未发现血成分, 受害者遭到剥皮放血: "
